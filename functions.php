...
// * Theme URI:
// * Description: This is a custom WordPress theme developed for specific purposes.
// * Author: Rifat
// * Author URI:
// * Version: 1.0
...
require_once ("redux/installer.php");
require_once ("redux/option-panel.php");
require_once ("redux/metabox.php");
...
function simpleshop_subcategory_count_html($markup){
    if (get_theme_mod('simpleshop_homepage_display_categories_number') !='on'){
        return '';
    }
    return $markup;
...
function get_option_value ($key,$default = ''){
	if( class_exists('Redux')){
		return Redux::get_option('Option_panel',$key,$default);
	}else{
		$default;
	}
}
